...
neologger.log_this(f"API_KEY: {api_key}")
neologger.log_this(f"USER_ID: {user_id}")
...
async def initialise():
    ...
    asyncio.create_task(build_steam_database())
...
async def build_steam_database():
    ...
    with lock:
        ...
        if results:
            ...
            neologger.log_this_ok(f"Results!")
            ...
            for result in tqdm(results):
                ...
                if name != "":
                    try:
                        session.execute(
                            insert(models.SteamApps).values(appid=appid, name=name)
                        )
        ...
        session.commit()
        results = session.query(models.SteamApps).all()
        session.execute(
            insert(models.SteamAppsMetadata).values(
                size=len(results), timestamp=datetime.now()
            )
        )
        session.commit()
        session.close()
            ...
            try:
                ...
                session.execute(
                    insert(models.SteamUserApps).values(
                        appid=appid, now_playing=0, favourite=0
                    )
                )
            except SQLAlchemyError as e:
                # logging.error(e)
...
async def refresh():
    ...
    asyncio.create_task(build_steam_database())
    return JSONResponse(status_code=200, content={"message": "Refresh triggered!"})
...
async def steam_games(
    ...
    if playedOnly is not None and unplayedOnly is None:
        results = session.query(
            models.SteamUserApps, models.SteamApps, models.SteamUserAppsPlaytime
        ).filter(models.SteamUserAppsPlaytime.playtime_forever > 0)
    ...
    if favouritesOnly is not None:
        results = results.filter(models.SteamUserApps.favourite == 1)
    ...
    results = (
        ...
        .join(
            ...
            models.SteamUserAppsPlaytime.appid == models.SteamApps.appid,
        )
        .group_by(models.SteamUserAppsPlaytime.appid)
        .order_by(order_by)
    )
            ...
            results_list.append(
                {
                    ...
                    "last_played": convert_timestamp(
                        steam_user_apps_playtime.last_played
                    ),
...
async def steam_game_id(id: str):
    ...
    results = (
        ...
        .filter(models.SteamUserApps.id == id)
        .order_by(models.SteamUserAppsPlaytime.timestamp.desc())
        .first()
    )
    session.close()
    if not results:
        return JSONResponse(
            status_code=404, content="No data found for the requested ID."
        )
...
async def steam_game_id_favourite(id: str):
    ...
    result = (
        session.query(models.SteamUserApps).where(models.SteamUserApps.id == id).one()
    )

    if result:
        favourite = 1 if result.favourite == 0 else 0
        update_stmt = (
            update(models.SteamUserApps)
            .where(models.SteamUserApps.id == id)
...
async def steam_user_status():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            results = results.json()
            return JSONResponse(
                status_code=200, content=results["response"]["players"][0]
            )

        except httpx.HTTPError as e:

            return JSONResponse(status_code=e.response.status_code, content=repr(e))
